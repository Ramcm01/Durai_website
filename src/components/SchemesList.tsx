...
export const SchemesList: React.FC<SchemesListProps> = ({
  ...
  return (
    <section id="schemes" className="py-20 bg-[#f4f0e8] border-b border-[#e5e0d3]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header styled strictly according to jayasulochanachits.com */}
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#8e1426] bg-[#8e1426]/10 px-3.5 py-1.5 rounded font-['Source_Sans_3'] mb-3">
            <Gift className="w-4 h-4 text-[#8e1426]" />
              ...
              <button
                ...
                className={`px-5 py-2 rounded text-xs font-semibold transition-all cursor-pointer ${
                  selectedCategory === cat.id
                    ? 'bg-[#8e1426] text-white shadow-sm'
                    : 'bg-white text-[#34322d] border border-[#e5e0d3] hover:border-[#8e1426]'
                }`}
              >
                {cat.label}
              </button>
        ...
        {/* Festive Banner matching Jayasulochana feature styling */}
        <div className="mb-12 rounded-xl overflow-hidden shadow-sm border border-[#e5e0d3] bg-[#0f172a] text-white relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 items-center">
            ...
            <div className="lg:col-span-5 relative h-56 sm:h-64 lg:h-72 overflow-hidden bg-slate-900">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-[#0f172a]/95 via-[#0f172a]/40 to-transparent" />
              
              <div className="absolute bottom-4 left-4 right-4">
                <span className="inline-block px-3 py-1 rounded bg-[#d79628] text-[#2c1b1b] text-xs font-bold uppercase tracking-wider mb-1 font-['Source_Sans_3']">
                  ✨ {t.bannerBadge}
                </span>
                <p className="text-xs text-[#e5e0d3] font-medium font-['Source_Sans_3']">
                  {language === 'ta' 
            ...
            <div className="lg:col-span-7 p-6 sm:p-8 space-y-4 font-['Source_Sans_3']">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#d79628]/20 border border-[#d79628]/40 text-[#d79628] text-xs font-bold">
              ...
              <p className="text-xs sm:text-sm text-[#e5e0d3] leading-relaxed">
                {t.bannerDesc}
              </p>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-1">
                <div className="p-3 rounded-lg bg-white/10 border border-white/15 text-center">
                  <div className="text-[10px] text-[#e5e0d3]">
                    {language === 'ta' ? '1 சீட்டு முதிர்வு' : '1 Chit Payout'}
                  </div>
                  <div className="text-base sm:text-lg font-bold text-[#d79628] font-['Playfair_Display']">₹35,600</div>
                </div>
                <div className="p-3 rounded-lg bg-white/10 border border-white/15 text-center">
                  <div className="text-[10px] text-[#e5e0d3]">
                    {language === 'ta' ? '5 சீட்டுகள் முதிர்வு' : '5 Chits Payout'}
                  </div>
                  <div className="text-base sm:text-lg font-bold text-[#d79628] font-['Playfair_Display']">₹1,38,000</div>
                </div>
              ...
              {onOpenCircular && (
                <div className="pt-2">
                  <button
                    ...
                    className="text-xs text-[#d79628] hover:underline font-semibold cursor-pointer inline-flex items-center gap-1.5"
                  >
        ...
        {/* Schemes Grid styled like jayasulochanachits.com cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-['Source_Sans_3']">
          {filteredSchemes.map((scheme) => (
            <div
              key={scheme.id}
              className={`rounded-lg bg-white border transition-all duration-200 flex flex-col justify-between overflow-hidden shadow-xs hover:shadow-md relative ${
                scheme.chitUnits === 1
                  ? 'border-[#8e1426] ring-1 ring-[#8e1426]/20'
                  : scheme.chitUnits === 10
                  ? 'border-[#d79628] ring-1 ring-[#d79628]/30'
                  : 'border-[#e5e0d3] hover:border-[#8e1426]/40'
              }`}
            >
              {/* Highlight bar matching jayasulochanachits.com */}
              {scheme.chitUnits === 1 && (
                <div className="bg-[#8e1426] text-white font-bold text-xs py-1 px-4 text-center uppercase tracking-wider font-['Source_Sans_3']">
                  ★ {language === 'ta' ? 'அடிப்படை 1 சீட்டு திட்டம் (முதிர்வு: ₹35,600)' : 'Standard 1 Chit Plan (Payout: ₹35,600)'}
                </div>
              )}
              {scheme.chitUnits === 10 && (
                <div className="bg-[#d79628] text-[#2c1b1b] font-bold text-xs py-1 px-4 text-center uppercase tracking-wider font-['Source_Sans_3']">
              ...
              <div className="p-6">
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <span className="text-[11px] font-bold text-[#8e1426] uppercase tracking-wider block">
                      {scheme.code} • 4 {t.monthsSuffix}
                    </span>
                    <h3 className="font-['Playfair_Display'] text-xl font-bold text-[#2c1b1b] mt-0.5">
                      {language === 'ta' && scheme.tamilName ? scheme.tamilName : scheme.name}
                    </h3>
                  </div>
                  <span className="px-2.5 py-1 rounded text-xs font-bold bg-[#faf8f4] text-[#8e1426] border border-[#e5e0d3]">
                    {scheme.chitUnits} {scheme.chitUnits === 1 ? (language === 'ta' ? 'சீட்டு' : 'Chit Unit') : (language === 'ta' ? 'சீட்டுகள்' : 'Chits')}
                  </span>
                </div>

                {/* Primary Payout Box */}
                <div className="bg-[#faf8f4] p-4 rounded-lg border border-[#e5e0d3] my-4 space-y-2.5">
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs text-[#57534e] font-semibold">{t.colMonthly}</span>
                    <span className="text-xl font-bold text-[#8e1426] font-['Playfair_Display']">
                      {formatCurrency(scheme.monthlyInstallment)}/{t.perMonth}
                    </span>
                  </div>

                  <div className="flex justify-between text-xs text-[#57534e] pt-1.5 border-t border-[#e5e0d3]">
                    <span>{t.col4MoDeposit}</span>
                    <span className="font-semibold text-[#2c1b1b]">
                      {formatCurrency(scheme.totalPrincipalDeposited)}
                    </span>
                  </div>

                  <div className="flex justify-between text-xs text-[#8e1426] font-semibold">
                  ...
                  <div className="p-3.5 rounded bg-[#0f172a] text-white shadow-xs mt-2 border-l-3 border-[#d79628]">
                    <div className="text-[11px] font-semibold text-[#d79628]">
                      {t.colTotalPayout}
                    </div>
                    <div className="text-2xl font-bold tracking-tight text-[#d79628] font-['Playfair_Display']">
                      {formatCurrency(scheme.totalCustomerPayout)}
                    </div>
                    <div className="text-[10px] text-[#e5e0d3] mt-0.5">
              ...
              <div className="p-6 pt-0 space-y-2">
                ...
                <button
                  ...
                  className="w-full py-2 px-4 rounded border border-[#e5e0d3] hover:border-[#8e1426] text-[#34322d] font-semibold text-xs flex items-center justify-center gap-1.5 cursor-pointer transition-colors bg-white hover:bg-[#faf8f4]"
                >
