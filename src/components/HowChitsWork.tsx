...
export const HowChitsWork: React.FC<HowChitsWorkProps> = ({ onOpenCircular, onOpenCalculator }) => {
  ...
  return (
    <section id="how-it-works" className="py-20 bg-[#140727] text-white border-b border-[#ede6f5]/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-purple-900/50 text-[#fde047] text-xs font-semibold mb-3 border border-[#f59e0b]/40 font-['Source_Sans_3']">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 font-['Source_Sans_3']">
          {steps.map((item) => {
            ...
            return (
              <div
                key={item.step}
                className="relative bg-white/5 rounded-xl p-6 border border-purple-500/20 flex flex-col justify-between hover:border-[#f59e0b]/60 hover:bg-white/10 transition-all text-center group"
              >
                <div>
                  <div className="w-16 h-16 rounded-full bg-[#581c87] flex items-center justify-center mx-auto mb-4 text-white shadow-md border-2 border-[#f59e0b]/50 group-hover:scale-105 transition-transform">
                    <Icon className="w-7 h-7 text-[#fde047]" />
                  </div>

                  <span className="text-xs font-bold text-[#fde047] uppercase tracking-widest font-mono block mb-1">
                    Step {item.step}
                  </span>
                  ...
                  <p className="text-xs font-semibold text-[#fde047] mb-3">
                    {item.subtitle}
                  </p>
        ...
        <div className="mt-12 bg-[#220d3f] rounded-xl p-6 sm:p-8 text-white border border-purple-500/30 shadow-xl grid grid-cols-1 md:grid-cols-12 gap-6 items-center font-['Source_Sans_3']">
          <div className="md:col-span-8 space-y-2">
            <div className="text-xs font-semibold text-[#fde047] uppercase tracking-wider flex items-center gap-1.5">
              <Building2 className="w-4 h-4 text-[#fde047]" />
            ...
            <div className="text-xs sm:text-sm text-[#e9d5ff] space-y-1">
              <div>• 1 Chit: 16,000 X 60% = ₹9,600 + ₹10,000 Referral = <strong className="text-[#fde047]">₹35,600 Payout</strong></div>
          ...
          <div className="md:col-span-4 flex flex-col sm:flex-row md:flex-col gap-2.5">
            {onOpenCircular && (
              <button
                ...
                className="w-full py-2.5 px-4 rounded-lg border border-[#f59e0b] hover:bg-[#f59e0b]/10 text-[#fde047] font-semibold text-xs flex items-center justify-center gap-2 cursor-pointer transition-colors"
              >
            ...
            {onOpenCalculator && (
              <button
                ...
                className="w-full py-2.5 px-4 rounded-lg bg-[#581c87] hover:bg-[#4c1d95] text-white font-semibold text-xs flex items-center justify-center gap-2 cursor-pointer shadow-xs transition-colors border border-[#f59e0b]/40"
              >
